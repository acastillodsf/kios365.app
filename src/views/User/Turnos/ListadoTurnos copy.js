import { CBadge, CBreadcrumbRouter, CButton, CCol, CInput, CModal, CModalBody, CPagination, CRow, CSelect } from '@coreui/react';
...
export const ListadoTurnos = ({ ws }) => {
    const ListTiket = useSelector((state) => state.ws.tiket);
    ...
    const GenerarLlamadoTiket = (data) => {
        return new Promise((resolve, reject) => {
            console.log('Data a Enviar =>', { type: "CALL_TICKET", data })
            // Enviar los datos al servidor a través del WebSocket
            ws.send(JSON.stringify({ type: "CALL_TICKET", data }));
