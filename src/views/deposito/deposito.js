...
const Deposito = () => {
  ...
  const { cuenta, divisa, monto, comentario, total, fee, token } = Transaccion;
  ...
  useEffect(()=>{
    if(wallet.Purchase.status==="OK"){
      if(wallet.Purchase.response.status==="Approved"){
        history.push(`/Transaccion/TransCard/Orden/${wallet.Purchase.response.order}`);
      }else{
        swal({
          title: "Transaccion",
          text: `Su transaccion ha sido rechazada, /n ${wallet.Purchase.response.descripcion} `,
          icon: "info",
          button: "Aceptar",
        });
      }
      dispatch({type:"wallet-Purchase->sucess"})
    }
  },[wallet])
  ...
  return (
    <>
      <CCol xs="12" sm="6">
        <CCard>
          { wallet.Purchase.status==="init" && <Downloading value="Procesando Pago, porfavor espere..." />}
                ...
                <CFormGroup row>
                  ...
                  <CCol xs="12" md="9">
                    ...
                    >
                      ...
                      {wallet.SettingCarNet.PaymentProfiles &&
                        wallet.SettingCarNet.PaymentProfiles.map((row) => {
                          return (
                            <option value={row.Token}>
                              {` **** **** ${row.Last4}`} | {row.Brand} |{" "}
                              {row.Expiration.substring(0, 4)} /{" "}
                              {row.Expiration.substring(4, 6)} | {row.CardOwner}
                            </option>
                          );
                        })}
                    </CSelect>
            ...
            {uepapayLoading ? (
              ...
              <>
                <CButton
                  ...
                  size="sm"
                  color="success"
                  disabled={monto.total === 0 || Transaccion.cuenta === ""}
                >
